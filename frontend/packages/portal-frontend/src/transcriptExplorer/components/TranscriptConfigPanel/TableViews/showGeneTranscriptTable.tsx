...
//
// Scoped by resolving the gene's transcripts up front and handing them to
// SliceTable as `rowIds`, rather than by having `implicitFilter` read the Gene
// column through `getValue`. That reads as the more direct way to write it and
// does not work: `getValue` resolves against the table's LOADED columns (see its
// own comment in @depmap/slice-table), so a column nobody asked to display is
// never fetched and comes back undefined -- which compares equal to no gene at
// all, and the table silently shows nothing. Loading Gene as a column would fix
// it too, at the cost of a column repeating the same symbol on every row of a
// modal already titled with it.
//
// `rowIds` rather than an id-testing `implicitFilter` because the two agree on
// what is displayed but not on what is transferred: `rowIds` pushes the set into
// the fetch. This table is where that matters most -- the `sequence` and
// `topology` columns are around 75MB each, and a gene has a handful of
// transcripts out of the whole dimension type.
  ...
  return (
    <div className={styles.tableContainer}>
      <SliceTable
        ...
        rowIds={transcriptIds ?? undefined}
      />
    </div>
  );
}

...
class PrivateDatasetMetadata(Model):
    """
    Adding a column to this table reuqires adding the column to many places. These include at lesat
        - the TypedDict definition
        - the to_dict function
        - PRIVATE_DATASETS_MAP_COLUMNS
    """

    __tablename__ = "nonstandard_private_dataset_metadata"

    uuid = Column(String, primary_key=True, nullable=False)  # should never be listed?
    csv_path = Column(String, primary_key=True, nullable=False)
    display_name = Column(String, nullable=False)
    units = Column(String, nullable=False)
    feature_name = Column(String, nullable=False)
    is_transpose = Column(Boolean, nullable=False)
    cell_line_name_type: "Column[CellLineNameType]" = Column(
        db.Enum(CellLineNameType, name="CellLineNameType"), nullable=False
    )
    owner_id = Column(Integer, nullable=False)
    data_type = Column(String, nullable=False)
    priority = Column(String, nullable=True)

    # Keep in sync with columns defined in depmap.user_uploads.utils
    PrivateDatasetMetadataDict = TypedDict(
        "PrivateDatasetMetadataDict",
        {
            "dataset": str,
            "display_name": str,
            "units": str,
    ...
    def add(
        ...
        """
        Wrapper to add a PrivateDatasetMetadata object
        """
        private_dataset = cls(
            uuid=uuid,
        ...
        db.session.add(private_dataset)
        db.session.commit()
    ...
    def to_dict(self) -> PrivateDatasetMetadataDict:
        """
        this function exist for serialisation, so as to reflect this table in persistent bucket storageb
        """
        return {
            ...
            "cell_line_name_type": self.cell_line_name_type.name,
    ...
    def get_by_dataset_id(
        dataset_id: str, must: bool = True
    ) -> Optional["PrivateDatasetMetadata"]:
        # WARNING: This CANNOT BE CACHED and must be re-retrieved on every request.

        query = PrivateDatasetMetadata.query.filter_by(uuid=dataset_id)
        if must:
            return query.one()
        else:

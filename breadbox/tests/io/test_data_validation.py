...
def test_verify_unique_rows_and_cols():
    ...
    with pytest.raises(FileValidationError) as ex:
        verify_unique_rows_and_cols(
            pd.DataFrame(
                0, columns=["C1", "C2", "C1"], index=["A", "B", "C"]
            )  # pyright: ignore
        )
    ...
    verify_unique_rows_and_cols(
        pd.DataFrame(
            0, columns=["C1", "C2", "C3"], index=["A", "B", "C"]
        )  # pyright: ignore
    )
    ...
    with pytest.raises(FileValidationError) as ex:
        ...
        verify_unique_rows_and_cols(
            pd.DataFrame(
                0, columns=columns + columns, index=["A", "B", "C"]
            )  # pyright: ignore
        )
    assert (
...
def test_read_and_validate_matrix_df(tmpdir):
    ...
    df = read_and_validate_matrix_df(
        to_csv(
            pd.DataFrame("3", columns=["C1", "C2"], index=["A", "B"])
        ),  # pyright: ignore
    ...
    df = read_and_validate_matrix_df(
        to_csv(
            pd.DataFrame("0", columns=["C1", "C2"], index=["A", "B"])
        ),  # pyright: ignore
    ...
    df = read_and_validate_matrix_df(
        to_csv(
            pd.DataFrame("0", columns=["10", "11"], index=["0", "1"])
        ),  # pyright: ignore
        ValueType.continuous,
        None,
        "csv",
...
def test_validate_tabular_df_schema(tmpdir):
    ...
    df = _validate_tabular_df_schema(
        to_csv(
            pd.DataFrame("0", columns=["13", "10", "11"], index=[1]), index=False
        ),  # pyright: ignore
        {

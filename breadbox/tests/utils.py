...
def assert_task_success(response):
    assert_status_ok(response)
    task_status = response.json()

    assert (
        task_status["state"] == "SUCCESS"
    ), f"expected failure, but status was {task_status}"


def assert_task_failure(response, status_code=None, message_contains=None):
    ...
    if message_contains is not None:
        assert (
            message_contains in task_status["message"]
        ), f"Expected {repr(message_contains)} in message but got {repr(task_status['message'])}"

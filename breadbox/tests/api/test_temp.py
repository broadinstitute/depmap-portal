...
def test_user_settings(client: TestClient, settings):
    ...
    response = client.get("/temp/user/settings", headers=user1_headers)
    assert response.status_code == 200
    assert {} == response.json()

    # make sure storing and getting results in same value
    value = {"name": "Joe", "age": 23, "contexts": [1, 2, 3]}
    response = client.post("/temp/user/settings", json=value, headers=user1_headers)
    assert response.status_code == 200
    ...
    response = client.post("/temp/user/settings", json=value2, headers=user1_headers)
    assert response.status_code == 200
    assert value2 == response.json()

    # now write as a different user and confirm those changes don't affect the first user
    response = client.get("/temp/user/settings", headers=user2_headers)
    assert response.status_code == 200
    assert {} == response.json()

    user2_value = {"name": "Martha"}
    response = client.post(
        "/temp/user/settings", json=user2_value, headers=user2_headers
    )
    assert response.status_code == 200

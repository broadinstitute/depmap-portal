...
- **Phase 1 — Scaffolding + first elements**: entry point/mounting contract, initial data fetch, loading state, the error banner (section 1), and the overview row (section 2: Aggregate Scores tile, GeneTEA tiles — CRISPR and RNAi). Also build a manual-testing stub page (see "Manual test page" below) so the component can be rendered and inspected without a real production entry point.

### Manual test page (Phase 1)

For manual verification during development, add a dev-only route `/test-pred-component/<dim_type>/<given_id>` that renders the page shell (nav + a mount div) and mounts the component with those two URL segments as props. Follow the **self-mounting webpack-entry idiom** used by `/celligner` and `/custom_analyses` (not the `window.DepMap.initXxx(...)` global-function idiom used elsewhere in `index.tsx` — the two are different patterns in this codebase; this stub page should use the former since that's the precedent to copy):

- **Flask route**: a new blueprint (or a route added to an existing dev-only blueprint) at `portal-backend/depmap/.../views.py`, e.g. `@blueprint.route("/<dim_type>/<given_id>")`, mirroring the multi-segment param precedent at `portal-backend/depmap/tile/views.py:59`. The view passes `dim_type`/`given_id` into `render_template(...)`.
- **Template**: a new `templates/test_pred_component/index.html`, following `templates/celligner/index.html` — extend `nav_footer.html` (so nav is included for free), add a mount div (e.g. `<div id="test-pred-component-root"></div>`), embed `dim_type`/`given_id` in a `<script id="test-pred-component-data" type="application/json">{ "dimType": {{ dim_type|tojson|safe }}, "givenId": {{ given_id|tojson|safe }} }</script>` block, then `<script src="{{ webpack_url('test_pred_component.js') }}"></script>`.
- **Frontend entry**: a new self-mounting file `frontend/packages/portal-frontend/src/apps/test_pred_component.tsx`, mirroring `apps/celligner.tsx`/`apps/custom_analyses.tsx` — read/`JSON.parse` the `#test-pred-component-data` script tag's `textContent`, then `ReactDOM.render` the Predictive Insights component (or call its `initPredictiveInsights`-equivalent mount function) into `#test-pred-component-root`.
- **Webpack**: register the new entry in `frontend/packages/portal-frontend/webpack.common.js` (e.g. `test_pred_component: "./src/apps/test_pred_component.tsx",`), alongside the existing `celligner`/`custom_analyses` entries.
- This route/page is dev/QA tooling, not a production entry point — keep it minimal (no auth/access-control beyond whatever the rest of the app already requires for a logged-in dev).

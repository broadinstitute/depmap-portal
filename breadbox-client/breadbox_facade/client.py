...
def _raise_for_non_json_error(response: httpx.Response) -> None:
    """
    Fail fast with a clear message when an error response isn't JSON.

    Auth proxies in front of breadbox (e.g. OAuth2 Proxy) can reject a request
    before it ever reaches breadbox and return an HTML sign-in page instead of
    breadbox's documented JSON error schema. The generated client always calls
    response.json() on error status codes, so without this check that HTML
    body causes an opaque json.JSONDecodeError instead of a usable message.
    """
    if response.status_code >= 400:
        content_type = response.headers.get("content-type", "")
        if "application/json" not in content_type:
            response.read()  # event hooks fire before the body is downloaded
            raise BreadboxException(
                response.status_code,
                f"Non-JSON error response from breadbox (content-type={content_type!r}): "
...
class BBClient:
    ...
    def __init__(self, base_url: str, user: str, password: Optional[str] = None):
        ...
        autogenerated_client = Client(
            ...
            httpx_args={"event_hooks": {"response": [_raise_for_non_json_error]}},

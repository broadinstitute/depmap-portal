...
def transform_cngene_to_log2(cngene_dataset_id: str) -> pd.DataFrame:
    ...
    filtered_cngene_expression_data = cngene_expression_data[
        cngene_expression_data["IsDefaultEntryForModel"] == "Yes"
    ].copy()

    assert (
        not filtered_cngene_expression_data["ModelID"].duplicated().any()
    ), "Duplicate ModelID after filtering"
    ...
    existing_cols_to_drop = [
        c for c in cols_to_drop if c in filtered_cngene_expression_data.columns
    ]
    filtered_cngene_expression_data = filtered_cngene_expression_data.drop(
        columns=existing_cols_to_drop
    )

    print("Setting ModelID as index...")
    filtered_cngene_expression_data = filtered_cngene_expression_data.set_index(
        "ModelID"
    )
    ...
    if count_all_na_columns > 0:
        ...
        filtered_cngene_expression_data = filtered_cngene_expression_data.dropna(
            axis=1, how="all"
        )

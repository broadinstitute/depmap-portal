...
def main(
    ...
    subtype_context_matrix_taiga_id = get_id(taiga_ids["subtype_context_matrix_taiga_id"])
    ...
    overall_summary.loc[
        overall_summary.index.isin(Repurposing_Matrix.columns),
        'PRISMRepurposing'
    ] = True
    ...
    if prism_oncref_auc_matrix is not None:
        ...
        overall_summary.loc[
            overall_summary.index.isin(OncRef_Matrix.index),
            'PRISMOncRef'
        ] = True
        resulting_cols.insert(-1, 'PRISMOncRef') #put the oncref column before the repurposing column
    ...
    overall_summary = overall_summary.merge(
        omics_summary, how='left',
        left_index=True, right_index=True
    )

    # Formatting
    overall_summary = overall_summary.fillna(
        False
    ).astype(
        bool
    ).loc[
    ...
    assert set(overall_summary.index) == set(context_matrix.index), \

This directory contains the various pipelines which need to be run to get files needed by the legacy portal loaded into GCS and the
Taiga dataset updated with some derived data.

Unfortuantely, the process of running the pipeline is complicated due to the number of dependencies it has. There's multiple layers starting with kicking off the job in jenkins:

- The "preprocessing" job in jenkins is run.
...
- if the command is one of the pipelines (ie `data-prep-pipeline/data_prep_pipeline_runner.py`) it will run various steps:
...
```
DEPMAP_DEPLOY_BRANCH=qa ./run.sh python data-prep-pipeline/data_prep_pipeline_runner.py \
   --deploy-name iqa \

...
def process_depmap_ipts(expr_df, context_df, prof_map, model_condition):
    ...
    hgnc_complete_set['depmap'] = hgnc_complete_set.symbol + ' (' + hgnc_complete_set.entrez_id.astype('Int64').astype(
        str) + ')'

    # hgnc_complete_set = hgnc_complete_set[hgnc_complete_set.locus_group == 'protein-coding gene']
    expr_ensembls = expr_df.columns.to_series()[expr_df.columns.to_series().str[:4] == 'ENSG']
    expr_nonsembls = expr_df.columns.to_series()[expr_df.columns.to_series().str[:4] != 'ENSG']

    expr_ensembls = expr_ensembls.str.extract(r'(ENSG[0-9]+)')
    expr_nonsembls.loc[expr_nonsembls.index.intersection(pd.Index(hgnc_complete_set.depmap))] = hgnc_complete_set.set_index('depmap').loc[expr_nonsembls.index.intersection(pd.Index(hgnc_complete_set.depmap))].ensembl_gene_id
